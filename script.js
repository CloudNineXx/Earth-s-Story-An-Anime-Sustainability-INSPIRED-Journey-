...
class SustainabilityWebsite {
  constructor() {
    ...
    // Audio controls
    this.hideControlsBtn = document.getElementById("hide-audio-controls");
  ...
  setupControlsToggle() {
    this.hideControlsBtn.addEventListener("click", () => {
      this.toggleControlsVisibility();
    });
    this.audioControls.classList.add("translucent");
  }
  ...
  setupFirstInteraction() {
    ...
    const startAudio = () => {
      if (!this.hasUserInteracted) {
        ...
        // Play current slide audio if exists
        const currentSlideElement = this.slides[this.currentSlide];
        if (currentSlideElement && this.slideAudioEnabled) {
  ...
  playSlideAudio(slideIndex) {
    if (!this.slideAudioEnabled || !this.hasUserInteracted) return;
